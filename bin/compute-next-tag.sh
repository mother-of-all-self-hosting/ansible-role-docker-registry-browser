...
#
# Tags look like `v<Docker Registry Browser version>-<release>`:
#
# - if defaults/main.yml points at a Docker Registry Browser version that has
#   never been released, the release counter restarts at 0 (`v1.10.0-0`)
# - otherwise the counter is incremented (`v1.10.0-1`), but only if something
...
# Anchored at the start of the line, so that neither a commented-out version
# nor any of the variables derived from this one (they mention it inside a
# Jinja expression rather than at the start of a line) can be picked up
# instead. This is the leaf literal Renovate rewrites, which is what makes it
# the only value here that is a version rather than an expression.
version="$(sed -nE 's|^docker_registry_browser_version:[[:space:]]*"?([^"[:space:]]+)"?.*$|\1|p' "$defaults_path" | head -n1)"

if [ -z "$version" ]; then
	echo >&2 "Could not determine the Docker Registry Browser version from $defaults_path"
	exit 1
fi

# The version value carries no leading `v` (e.g. `1.10.0`) while the tags do,
# but strip one anyway so that either shape produces the same tag.
