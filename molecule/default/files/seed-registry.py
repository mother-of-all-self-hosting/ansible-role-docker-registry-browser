...
A Docker Registry Browser with nothing to browse renders an error page, so
the registry it is pointed at has to hold something before the browse
pipeline can be exercised at all. This is what puts it there.
...
- Docker only pushes what its image store holds, and a `docker push` of the
  ...
  digest. The browser lists tags by digest as well as by name, so identical
  digests would make the per-tag assertions in verify.yml meaningless. Every
  tag here gets a layer of its own, carrying its own name, and therefore a
  digest of its own.

- Recent Docker releases push OCI manifests, which answer a request for
  `application/vnd.docker.distribution.manifest.v2+json` with a 404. The
  browser asks for exactly that media type, so what a `docker push` produced
  would depend on the Docker version the test happened to run against.
  Speaking the API directly pins the media type.

The image config below carries a `created` timestamp, which a hand-built
image config can otherwise leave out. The browser's tag detail page renders
it unconditionally (`app/views/manifests/sections/_details.html.erb`) and
answers 500 when it is missing, which would take the delete-control
assertions in verify.yml down with it.
"""
